...
func testDebug(msg string, args ...interface{}) {
	str := fmt.Sprintf("%s \n", msg)
	fmt.Printf(str, args...)
}
...
func TestReElection2A(t *testing.T) {
	...
	testDebug("disconnecting leader %d", leader1)
	cfg.disconnect(leader1)
	cfg.checkOneLeader()

	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	testDebug("reconnecting oldleader %d", leader1)
	cfg.connect(leader1)
...
func TestFailAgree2B(t *testing.T) {
	...
	testDebug("disconnecting follower: %d", (leader+1)%servers)
	...
	// re-connect
	testDebug("reconnecting follower: %d", (leader+1)%servers)
...
func TestFailNoAgree2B(t *testing.T) {
	...
	leader := cfg.checkOneLeader()
	testDebug("disconnecting 3 servers")
	cfg.disconnect((leader + 1) % servers)
	...
	testDebug("reconnecting back 3 servers")
	// repair
	cfg.connect((leader + 1) % servers)
...
loop:
	for try := 0; try < 5; try++ {
		...
		if total2-total1 > (iters+1+3)*3 {
			t.Fatalf("too many RPCs (%v) for %v entries\n max: %d", total2-total1, iters, (iters+1+3)*3)
		}
